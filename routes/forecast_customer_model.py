...
app = Flask(__name__)
customer_model_bp = Blueprint('customer-model', __name__)
# Connect to MongoDB
...
customer_model_path = os.path.join(model_directory, 'customer_model.joblib')

if os.path.exists(customer_model_path):
    customer_model = joblib.load(customer_model_path)
else:
    raise FileNotFoundError(f"Profit model not found at {customer_model_path}")

@customer_model_bp.route('/forecast/customer', methods=['POST'])
def forecast_production():
    ...
    customer_forecast = customer_model.forecast(steps=steps)

    # Construct response data
    response_data = {
        'customer_forecast': customer_forecast.tolist()  # Convert to list for JSON serialization

...
app = Flask(__name__)
profit_model_bp = Blueprint('profit-model', __name__)
# Connect to MongoDB
...
profit_model_path = os.path.join(model_directory, 'profit_model.joblib')

if os.path.exists(profit_model_path):
    profit_model = joblib.load(profit_model_path)
else:
    raise FileNotFoundError(f"Profit model not found at {profit_model_path}")

@profit_model_bp.route('/forecast/profits', methods=['POST'])
def forecast_profits():
    ...
    profits_forecast = profit_model.forecast(steps=steps)

    # Construct response data
    response_data = {
        'profits_forecast': profits_forecast.tolist()  # Convert to list for JSON serialization

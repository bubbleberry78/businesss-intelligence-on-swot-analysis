...
app = Flask(__name__)
production_model_bp = Blueprint('production-model', __name__)
# Connect to MongoDB
...
production_model_path = os.path.join(model_directory, 'production_model.joblib')

if os.path.exists(production_model_path):
    production_model = joblib.load(production_model_path)
else:
    raise FileNotFoundError(f"Profit model not found at {production_model_path}")

@production_model_bp.route('/forecast/production', methods=['POST'])
def forecast_production():
    ...
    production_forecast = production_model.forecast(steps=steps)

    # Construct response data
    response_data = {
        'production_forecast': production_forecast.tolist()  # Convert to list for JSON serialization
